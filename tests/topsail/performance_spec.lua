...
spec:
  ...
  describe("memory usage performance", function()
    it("should not leak memory during repeated operations", function()
      ...
      assert.is_true(
        memory_increase < 10240,
        string.format("Memory increased by %.2fKB, expected < 10MB", memory_increase)
      )
    end)
    ...
    it("should handle concurrent operations efficiently", function()
      ...
      assert.is_true(
        duration_ms < 3000,
        string.format("Concurrent operations took %dms, expected < 3000ms", duration_ms)
      )
